...
from elmo_geo.st.index import index_centroid, index_chipped
...
def to_gpq_sorted(sdf:SparkDataFrame, sf_out:str):
	...
	'''
	(sdf
		.transform(index_centroid, resolution='1km')
		.sort('sindex')
		.write.format('geoparquet')
		.save(sf_out)
	)
...
def to_gpq_zsorted(sdf:SparkDataFrame, sf_out:str):
	...
	'''
	(sdf
		.transform(index, resolution='1km', index_join=chipped_index)
