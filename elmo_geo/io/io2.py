...
def PandasDataFrame_to_GeoDataFrame(pdf:PandasDataFrame, column:str, crs:Union[int,str]) -> GeoDataFrame:
  return GeoDataFrame(pdf, geometry=GeoSeries.from_wkb(pdf[column], crs=crs), crs=crs).drop(columns=[column])

def SparkDataFrame_to_GeoDataFrame(df:SparkDataFrame, column:str, crs:Union[int,str]) -> GeoDataFrame:
  return (SparkDataFrame_to_PandasDataFrame(df)
...
def ingest_file(f_in:str, f_gpq:str, f_pq:str, batchsize:int=BATCHSIZE, spark:SparkSession=None):
