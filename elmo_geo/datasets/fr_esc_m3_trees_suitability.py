...
def _calculate_mean_suitability(sdf: SparkDataFrame, woodland_type: str) -> SparkDataFrame:
    ...
    """
    return sdf.groupBy(["RLR_RW_REFERENCE_PARCELS_DEC_21_LPIS_REF", "period_AA_T1", "period_T2"]).agg(
...
def _convert_to_wide_format(sdf: SparkDataFrame, woodland_type: str) -> SparkDataFrame:
    ...
    """
    return (
        sdf.withColumn("T1_T2", F.expr("CONCAT('T1_',period_AA_T1, '_T2_', period_T2 )"))
...
def _transform_single_woodland_type(sdf: SparkDataFrame, woodland_type: str) -> SparkDataFrame:
    ...
    return (
        sdf.transform(_convert_to_long_format)
...
def _transform(
    ...
    return (
        _transform_single_woodland_type(esc_broadleaved.sdf(), "broadleaved")
        .join(_transform_single_woodland_type(sdf_coniferous, "coniferous").withColumnRenamed("nopeatArea", "npaCon"), on=["id_parcel"])
        .join(_transform_single_woodland_type(esc_riparian.sdf(), "riparian").withColumnRenamed("nopeatArea", "npaRip"), on=["id_parcel"])
