...
# Hedge
hedge_boundaries = DerivedDataset(
    level0="silver",
    level1="elmo_geo",
    name="hedge_boundaries",
    model=SjoinBoundaries,
    restricted=True,
    func=sjoin_boundary,
...
water_boundaries = DerivedDataset(
    level0="silver",
    level1="elmo_geo",
    name="water_boundaries",
    model=SjoinBoundaries,
    restricted=True,
    func=partial(sjoin_boundary, fn_pre=fn_pre_water),
    dependencies=[boundary_segments, os_ngd_raw],
    is_geo=False,
)


# Wall - no os wall data
# Merge
