...
class OSMRaw(DataFrameModel):
    ...
    layer: Category = Field(coerce=True, isin=["points", "lines", "multilinestrings", "multipolygons", "other_relations"])
    fid: int = Field(coerce=True, unique=True, alias="osm_id")
    geometry: Geometry(crs="EPSG:4326") = Field(coerce=True)
...
def _osm_transform(sdf: PandasDataFrame) -> PandasDataFrame:
    ...
    return (
        ...
        .mapInPandas(
            lambda pdf: pdf.assign(json_tags=pdf.apply(_cols_to_json, axis=1).astype(str))[["osm_id", "json_tags", "geometry"]],
...
# Parcels
class OSMParcels(DataFrameModel):
    """Model for OSM attached to parcels.

    Attributes:
        id_parcel: 11 character RPA reference parcel ID (including the sheet ID) e.g. `SE12263419`.
        group: created by osm_tidy
        proportion: proportion of parcel geometry overlapping with geometry.
    """
