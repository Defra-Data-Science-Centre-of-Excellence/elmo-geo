...
farm_type = (parcel
  .merge(df_farm, how='left')
  .assign(
    category_group = 'farm_type',
    category = lambda df: df['farm_type'].map({
      0: None,
      1: 'Cereals',
      2: 'General Cropping',
      6: 'Dairy',
...
priority_habitat = (parcel
  .merge(df_priority_habitat, how='left')
...
df = (df_cats
  .merge(df_base)
  .groupby(['feature', 'category_group', 'category', 'metric'])
  ['value']
  .agg(['sum', 'mean'])
  .reset_index()
)
