...
from cdap_geo.sedona import st_join, st_valid, st_fromwkb
...
sdf_parcels = (spark.read.parquet(sf_parcels)
...
sdf_geoms = (spark.read.parquet(sf_rpa_hedge)
...
sdf_rpa_hedge = (spark.read.parquet(sf_geoms_out)
...
display(spark.read.parquet(sf_rpa_hedge)
  .withColumn('geometry', st_fromwkb('wkb_geometry', 27700))
  .select(
...
display(spark.read.parquet(sf_out).select(
  F.sum('m_length').alias('m_efa'),
  F.sum('m_adj'),
  F.sum('m_adj_elg'),
))

display(spark.read.parquet(sf_osm_hedge).select(
  F.sum(F.expr('ST_Length(geometry)')).alias('m_osm'),
))


display(sdf_osm_hedge.select(
...
df_parcel = (spark.read.parquet(sf_parcels)
  .filter(loc)
  .toPandas()
  .pipe(lambda df: gpd.GeoDataFrame(df, geometry='geometry', crs=crs))
)

df_hedge = (spark.read.parquet(sf_rpa_hedge)
  .select(
    'id',
    F.concat('REF_PARCEL_SHEET_ID', 'REF_PARCEL_PARCEL_ID').alias('id_parcel'),
...
df_parcel.plot(ax=ax, alpha=.2, color='darkgoldenrod')
