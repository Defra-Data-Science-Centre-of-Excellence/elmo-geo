...
df_parcel = spark.read.parquet(sf) \
  .select(
    'id_business',
    'id_parcel',
    'parcel_area',
...
df_business = df_parcel \
  .assign(parcel_count=1) \
  .groupby('id_business').sum().reset_index() \
  .replace(0, np.nan)
...
# Scenario 4
lengths, areas = df_business_hedge['hedge_length'], df_business_hedge['parcel_area'];  lengths_per_area = lengths / areas
elig = areas.sum()
...
# Scenario 4
lengths, areas = df_business_water['water_length'], df_business_water['parcel_area'];  lengths_per_area = lengths / areas
elig = areas.sum()
