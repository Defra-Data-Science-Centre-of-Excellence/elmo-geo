...
from cdap_geo import st_join, area, intersection_area, to_gdf
...
df_parcels = spark.read.parquet(f).select(
...
df = st_join(
  ...
  'area_overlap', area(intersection('geometry_left', 'geometry_right'))
).filter(
  '0 < area_overlap'
).withColumn(
  'prop', F.col('area_overlap') / F.col('area')
).select(
  'id_left',
  'id_right',
  'area',
...
gdf = to_gdf(df_parcels.filter(F.col('id_parcel').isin(ids)), crs=27700) \
  .merge(df2[['id_left', 'prop']].rename(columns={'id_left':'id_parcel'}))
...
for i, ax in zip(x, axs):
  ...
  ax = df.plot(ax=ax, alpha=.3, edgecolor='k')
  ax.set(title=f'{b}, {s}')
  ax.axis('off')
...
df_left = (naturalearth_cities.copy()
  .rename(columns={'name': 'city'})
  [['city', 'geometry']]
)

df_right = (naturalearth_lowres.copy()
...
def knn_join(left, right, k=1):
  ...
  df = ( concat(geohash_join(left, right))
