...
def spatial_join(
    ...
    df = Adapter.toDf(result_pair_rdd, spark) # remove use of keyword arguments, argument names have changed.
    
    #df = df.toDF(*_get_column_names(df_left, df_right)) # This doesn't work on 3c. output from sptail join only has two fields 'leftgeometry' and 'rightgeometry'
