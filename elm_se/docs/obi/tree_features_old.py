...
def join_and_aggregate_trees_to_features(spark: SparkSession,
    ...
    featureName: str
                 The column name to use for the aggregated tree counts
    ...
    featureTreesDF = spatial_join(
                        ...
                        useIndex = True,
                        considerBoundaryIntersection = False, # returns tree geometries contained within hedgerow geoms (since trees are point geoms using 'True' should produce the result)
    ...
    featureTreesPerParcelDF = (featureTreesDF
                          .select("SHEET_PARCEL_ID", "geom_right")
                          .drop_duplicates(subset=subset)
                          .groupby("SHEET_PARCEL_ID")
                          .agg(F.count("geom_right").alias(featureName))
                        )
...
def feature_tree_and_parcel_counts(featuresDF: SparkDataFrame,
                                   ...
                                   featureName: str) -> Dict:
    """
    Get counts of feature trees and parcels with features and feature trees. Useful for reporting and testing.
    """
    nFeatureTrees = featureTreesDF.drop_duplicates(subset=["geom_right"]).count()

    df = featureTreesPerParcelDF.select(F.sum(featureName).alias("tot_parcel_ftrees")).toPandas()
    nFeatureParcelTrees = df["tot_parcel_ftrees"].values[0]

    nFeatureParcels = featuresDF.select("SHEET_PARCEL_ID").distinct().count() # Number of parcels with features
    nFeatureTreeParcels = featureTreesDF.select("SHEET_PARCEL_ID").distinct().count() # parcels with feature trees

    counts = {"nFeatureTrees":nFeatureTrees,
...
def get_hedgerow_trees_features(spark: SparkSession,
    ...
    hrDF = hrDF.withColumn("geometry", F.col("buffered_geom")) # buffered hedge linestring

    featureName = "hrtrees_count"
    hrtreesDF, hrtreesPerParcelDF = join_and_aggregate_trees_to_features(spark,
                                                                         treesDF,
                                                                         hrDF,
                                                                         featureName,
                                                                         double_count)
    ...
    nHRTreesDup = hrtreesDF.count()

    LOG.info(f"Number of hedgerow trees (number without duplicate geoms dropped): {counts['nFeatureTrees']} ({nHRTreesDup})")
    LOG.info(f"Number of hedgerow trees in parcels: {counts['nFeatureParcelTrees']}")
    LOG.info("\n")
    LOG.info(f"Number of parcels w hedgerows: {counts['nFeatureParcels']}")
    LOG.info(f"Number of parcels w hedgerow trees: {counts['nFeatureTreeParcels']}")
...
def get_waterbody_trees_features(spark: SparkSession,
    ...
    treesDF = treesDF.withColumn("geometry", F.col("top_point")) # tree top coordinate
    wbDF = wbDF.withColumn("geometry", F.col("geometry_water_buffered")) # buffered waterbody geometry
    ...
    wbTreesDF, wbTreesPerParcelDF = join_and_aggregate_trees_to_features(spark,
                                                                         treesDF,
                                                                         wbDF,
                                                                         featureName,
                                                                         double_count,
    ...
    LOG.info(f"Number of waterbody trees: {counts['nFeatureTrees']}")
    LOG.info(f"Number of waterbody trees in parcels: {counts['nFeatureParcelTrees']}")
    LOG.info("\n")
    LOG.info(f"Number of parcels w waterbodies: {counts['nFeatureParcels']}")
    LOG.info(f"Number of parcels w waterbody trees: {counts['nFeatureTreeParcels']}")
...
def get_perimeter_trees_features(spark: SparkSession,
    ...
    parcelsDF = parcelsDF.withColumn("geometry", F.col("perimeter_geom_buf")) # buffered parcel perimeter
    treesDF = treesDF.withColumn("geometry", F.col("top_point")) # tree top coordinate
    ...
    if double_count==False:
        pTreesDF = pTreesDF.drop_duplicates(subset=['geom_right'])

    # Get the length of the perimeter that is intersected by tree crowns
    pTreesDF = pTreesDF.withColumn("crown_perim_length", F.expr("""
                                                                ST_Length(
                                                                    ST_Intersection(
                                                                        ST_GeomFromWKT(crown_poly_raster), perimeter_geom
                                                                    )
                                                                )
                                                                """)
                                   )
    
    # Aggregate - get length of intersected perimieter and count of perimeter trees
    perimTreesPerParcelDF = (pTreesDF
    ...
    nTreeParcels = perimTreesPerParcelDF.select("SHEET_PARCEL_ID").distinct().count() # Number of parcels with trees in perimeter
...
def get_interior_trees_features(spark: SparkSession,
    ...
    double_count=False
    iTreesDF, intTreesPerParcelDF = join_and_aggregate_trees_to_features(spark,
                                                                         treesDF,
                                                                         parcelsDF,
                                                                         featureName,
                                                                         double_count
    ...
    # Log some results
    nIntTreeParcels = intTreesPerParcelDF.select("SHEET_PARCEL_ID").distinct().count() # Number of parcels with trees in interior
...
def get_parcel_tree_features(spark: SparkSession,
                             ...
                             wbDF: SparkDataFrame,
                             parcel_buffer_distance: float,
                             hedgerow_buffer_distance: float,
                             waterbody_buffer_distance: float,                             
                             double_count: Optional[bool]=False
                             )-> SparkDataFrame:
    """
    ...
    hedgerow_buffer_distance: float
                              The distance in meters to buffer hedgerow geometries by.

    waterbody_buffer_distance: float
                            The distance in meters to buffer waterbody geometries by.
    ...
    wbDF = wbDF.withColumn("geometry_water_buffered", F.expr(f"ST_Buffer(geometry_water, {waterbody_buffer_distance})"))
    ...
    parcelPerimTreesDF = get_perimeter_trees_features(spark, treesDF, parcelsDF, double_count=double_count)
    parcelInterTreesDF = get_interior_trees_features(spark, treesDF, parcelsDF)
    hrtreesDF, hrtreesPerParcelDF, hrCounts = get_hedgerow_trees_features(spark,treesDF,hrDF,double_count=double_count)
    wbTreesDF, wbTreesPerParcelDF, wbCounts = get_waterbody_trees_features(spark,treesDF,wbDF,double_count=double_count)
