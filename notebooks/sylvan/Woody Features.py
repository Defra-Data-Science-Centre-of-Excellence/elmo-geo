...
if filter_parcels:
  ...
  path_out = 'dbfs:/mnt/lab/unrestricted/elm/elm_se/relict_hedge/test'+filter_tile+'/elm_se-{}-2023_08.parquet' # v2 - changes to where segmenting happens (after hedge and wood priority) and joining segments back to original parcel boundararies. Output needs to be same geometry as parcel boundary.
...
sdf_parcel = (sdf_parcel
  ...
  .withColumn("geometry_boundary",F.expr('ST_MakeValid(ST_Boundary(geometry)) AS geometry_boundary'),)
)
...
def group_agg_features(sdf, geom:str, group_cols:list):
  sdf = (sdf
    .groupby(*group_cols) 
    .agg(F.expr(f'ST_MakeValid(ST_Union_Aggr({geom}))  AS geometry_tmp'))
  )
  return sdf

def boundary_use(left, right, left_geom:str, right_geom:str, suffix:str, buf:int=2, simp:int=0, group_cols:list = ['major_grid', 'bng_10km', 'bng_1km', 'id_parcel']):
  fn = """ST_MakeValid({opp}(
    {gb},
  ...
  return (left.join(right, on=group_cols, how='left')
...
sdf_woody_boundaries = (sdf_parcels
                        .select(*group_cols, 'geometry_boundary', F.expr('geometry_boundary AS geometry_boundary_orig'))
                        .transform(boundary_use, sdf_hedge, left_geom = 'geometry_boundary', right_geom='geometry_hedge', suffix = 'hedgerow_boundary', simp = 0, buf = 2, group_cols = group_cols)
                        .transform(boundary_use, sdf_woodland, left_geom = 'geometry_boundary', right_geom='geometry_woodland', suffix='woodland_boundary', simp = 0, buf = 2, group_cols = group_cols)
)

# Checkpoint. Record the boundary available. This is the geometry that will be converted into segments.
sdf_woody_boundaries = (sdf_woody_boundaries
                        .withColumn("geometry_boundary_relict_available", F.col("geometry_boundary")))
...
def apply_st_func(sdf, func, explode_query, gcol, **kwargs):
  ...
  '''
  (sdf
   .withColumn(gcol, F.expr(f'ST_AsBinary({gcol})'))
...
(sdf_woody_boundaries
 .withColumn("geometry_boundary_relict_available", F.expr(f"ST_MakeValid(ST_SimplifyPreserveTopology(geometry_boundary_relict_available, 1))"))
...
sdf_available_segments = (spark.sql(segment_query("available_boundaries"))
...
sdf_available_segments = (spark.read.parquet(sf_available_segments_out)
...
sdf_other_woody_vom = (
  st.sjoin(sdf_available_segments, sdf_vom_td, distance=BUFFER, lsuffix='_segjoin', rsuffix='_vom_td', spark=spark)
)
...
sdf_other_woody_segments = (sdf_available_segments
                             .select(*group_cols, 
...
sdf_woody_boundaries = (spark.read.parquet(sf_woody_boundaries_out)
                        .withColumn("geometry_boundary", F.col("geometry_boundary_relict_available"))
)
