...
hrtrees_output_template = ( "dbfs:/mnt/lab/unrestricted/elm/elmo/"
                "hrtrees/hrtrees_{timestamp}_{buf}mbuffer.parquet"
            )
...
parcel_hrtrees_csv_template =  "/dbfs/mnt/lab/unrestricted/elm/elmo/hrtrees/hrtrees_per_parcel_{timestamp}_{buf}mbuffer.csv"

output_trees_path = path_output_template.format(mtc='all', timestamp=timestamp)
hrtrees_path_output = hrtrees_output_template.format(timestamp=timestamp, buf=hedgerows_buffer_distance)
parcel_hrtrees_output = parcel_hrtrees_template.format(timestamp=timestamp, buf=hedgerows_buffer_distance)
parcel_hrtrees_csv_output = parcel_hrtrees_csv_template.format(timestamp=timestamp, buf=hedgerows_buffer_distance)

old_output_trees_path = "dbfs:/mnt/lab/unrestricted/elm/elmo/hrtrees/tree_detections/all-tree-detections.parquet"
...
hrtreesPerParcelDF = hrtreesPerParcelDF.join(
                                            hrLengthDF, 
                                            hrtreesPerParcelDF.SHEET_PARCEL_ID==hrLengthDF.id_parcel, 
                                            "left"
                                            )
hrtreesPerParcelDF = hrtreesPerParcelDF.withColumn("hrtrees_per_m", F.col("hrtrees_count")/F.col("hedge_length"))

# COMMAND ----------

# Get total coverage of hr trees
area = hrtreesDF.select( F.expr(f"""
                                                        ST_Area(
...
parcel_count = pd.Series(index=['All Parcels','HR Parcels','HR Tree Parcels'], data=[counts['nParcels'],counts["nHRParcels"], counts["nHRTParcels"]])
parcel_count

# COMMAND ----------

def stacked_bar_parcel_counts(
    data: pd.Series, title: str, n_unknown: Optional[int]=None,
    ...
    for i, (label, value) in enumerate(data.sort_values(ascending=False).items()):
        ...
        plt.text(
            p_val,y_pos,
            f"{value:,.0f}",
            fontdict={"fontsize":20, "color":"#5b5b5b"}
        )
    ...
    f.supxlabel(
        f"Source: Environment Agency Vegitation Object Model"+r" $1m^2$"+" and Rural Payments Agency EFA Hedges",
        x=0.09,
        y=-0.07,
        ha="left",
        fontsize="small",
...
f, ax = stacked_bar_parcel_counts(parcel_count, title="Proportions of parcels with hedgerows and hedgerow trees", n_unknown=n_unknown)
f.show()
...
data.loc[ data['hrtrees_count']> data['hrtrees_count'].quantile(0.995)].shape, data.loc[ data['hrtrees_per_m']> data['hrtrees_per_m'].quantile(0.995)].shape
...
data['SHEET_PARCEL_ID'].isnull().value_counts(), data['id_parcel'].isnull().value_counts(), data['hedge_length'].isnull().value_counts(), data['hrtrees_per_m'].isnull().value_counts()

# COMMAND ----------

def plot_hrtree_dist(
    data: pd.Series, title: str, xlabel: str, max_quantile: float, dark: bool = False, dp: int = 0, bin_ratio: int = 1
) -> Tuple[plt.Figure, plt.Axes]:
...
f, ax = plot_hrtree_dist(data["hrtrees_count"], "Distribution of hedgerow trees", "Number of hedgerow trees", 0.995, dark = False, dp=1)
f.show()

# COMMAND ----------

f, ax = plot_hrtree_dist(data["hrtrees_per_m"]*100, "Distribution of hedgerow tree density", "Hedgerow trees per 100m of hedgerow", 0.995, dark = False, dp=1, bin_ratio=3)
f.show()
...
hrParcelsDF = hrDF.withColumn("SPID", F.concat("REF_PARCEL_SHEET_ID","REF_PARCEL_PARCEL_ID")).drop_duplicates()
allDF = (hrParcelsDF
            .join(hrtreesPerParcelDF, hrParcelsDF.SPID==hrtreesPerParcelDF.SHEET_PARCEL_ID, 'left')
            .select(F.col("SPID").alias("SHEET_PARCEL_ID"), "hrtrees_count", "hrtrees_per_m")
            .fillna(0, subset=["hrtrees_count", "hrtrees_per_m"])
            .drop_duplicates()
        )
...
f, ax = plot_hrtree_dist(hrpdf_dd["hrtrees_per_m"]*100, "Distribution of hedgerow tree density - all parcels with hedgerows", "Hedgerow trees per 100m of hedgerow", 0.995, dark = False, dp=3, bin_ratio=3)
f.show()
...
hrSubDF = (hrSubDF
        ...
        .withColumn("geometry", F.expr(f"ST_Buffer(ST_GeomFromWKB(wkb), {hedgerows_buffer_distance})"))
)
treesSubDF = treesSubDF.withColumn("geometry", F.expr("ST_Point(top_x, top_y)"))

hrSubDF = hrSubDF.filter(F.expr(f"""
                                ST_Contains(
...
treesSubDF = treesSubDF.filter(F.expr(f"""
                                ST_Contains(
                                    ST_GeomFromWKT('{bbox.wkt}'),geometry
                                )
                                """)
...
hrtreesSubDF = hrtreesSubDF.filter(F.expr(f"""
                                ST_Contains(
...
ghrDFr['SHEET_PARCEL_ID'] = ghrDFr.apply(lambda row: f"{row['REF_PARCEL_SHEET_ID']}{row['REF_PARCEL_PARCEL_ID']}")
