...
# this mapping systematically excludes some Main Habit classes but that doesn't affect the analysis
...
ph_main_habits_not_mapped = (
    df_comp.loc[(df_comp["A_pred_from_Main_Habit"].isnull() & df_comp["Main_Habit"].notna())]
    .groupby("Main_Habit")["id_parcel"]
    .apply(lambda s: s.nunique())
    .sort_values()
    .to_dict()
)

hm_a_pred_not_mapped = (
    df_comp.loc[(df_comp["Main_Habit_from_A_pred"].isnull() & df_comp["A_pred"].notna())]
    .groupby("A_pred")["id_parcel"]
    .apply(lambda s: s.nunique())
    .sort_values()
    .to_dict()
)
...
frequency_comp = pd.merge(ph_frequency, hm_frequency, left_on="A_pred_from_Main_Habit", right_on="A_pred", how="outer", suffixes=("_ph", "_hm")).sort_values(
    "parcel_count_hm", na_position="first"
)
...
count_match = (
    df_comp.dropna(subset=["Main_Habit", "A_pred"])  # only parcels with a habitat from both datasets
    .groupby(["id_parcel", "A_pred"])["habitats_match"]
    .any()  # count number of parcels that have a habitat match for each A_pre category
    .reset_index()
    .groupby("A_pred")["habitats_match"]
    .value_counts()  # count total number of matches across haabitats
    .unstack()  # pivot
)
...
# plots showing which habitats most commonly overlap with parcels according to each dataset
sns.set(
    context="talk",
    style="white",
    palette="husl",
