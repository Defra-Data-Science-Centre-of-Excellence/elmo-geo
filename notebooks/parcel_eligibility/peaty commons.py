...
df_peat['shape_area'].sum()//x, df_commons['Shape__Area'].sum()//x, df.area.sum()//x, (df.area.sum()/df_commons['Shape__Area'].sum()).round(3)

# COMMAND ----------

fig, ax = plt.subplots(dpi=800)

df_peat.geometry.plot(ax=ax, alpha=.5, color='tab:brown')
