...
sdf_parcels = spark.read.format('geoparquet').load(sf_parcels_stg).select(
...
sdf_moor = spark.read.format('geoparquet').load(sf_moor_stg).select(
...
sdf_nfi = spark.read.format('geoparquet').load(sf_nfi_stg).select(
...
sdf = (sdf_parcels
  .transform(sjoin, sdf_moor, how='full', lsuffix='', rsuffix='_moor')
  ...
  .withColumn('geometry_overlap', F.expr('ST_Intersection(geometry, ST_Intersection(geometry_moor, geometry_nfi))'))
  # .select(
  #   # 'id_business',
  #   'id_parcel',
  #   'class_moor',
