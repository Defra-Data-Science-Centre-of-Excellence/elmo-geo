...
df_wfm = (pd.read_feather(f_wfm)
  [['id_business', 'id_parcel', 'ha_parcel_uaa']]
)
df_ph = (pd.read_feather(f_ph)
  [['id_parcel', 'Main_Habit', 'confidence', 'proportion']]
)
df = (df_wfm.merge(df_ph)
  .assign(
    ha = lambda df: (df['ha_parcel_uaa'] * (.1<df['proportion'])).replace(0, np.NAN),
...
df_ph = (df
  .groupby(['id_business', 'Main_Habit'])
  [['ha', 'count', 'ha_ph']]
  .sum().reset_index()
  .assign(
...
def kdeplots(df, x, y, rot):
  ...
  pal = sns.cubehelix_palette(df[y].nunique(), rot=rot, dark=.25, light=.75)
  ...
  def label(x, color, label):
    ax = plt.gca()
    ax.text(0, .2, label, fontweight='bold', color='k', ha='left', va='center', transform=ax.transAxes)
  g.map(label, x)
...
kdeplots(df, 'log_ha', 'Main_Habit', -.5)
kdeplots(df_ph, 'log_count', 'Main_Habit', 0)
kdeplots(df_ph, 'log_ha_ph', 'Main_Habit', .5)
