...
# MAGIC
# MAGIC Segmentation can be done at each node, it can also be done to split 2 points at a maximum length.  However, this can create very short segments (especially along bends and corners), and such simplification is also utilised to merge shorter segments together.
# MAGIC
...
from shapely import (
    LineString, MultiLineString, MultiPoint, Point,
    segmentize
)
...
from elmo_geo.io import to_parquet, to_gdf
...
def segmentize_with_tolerance(geometry: LineString, tolerance: float = 10, length: float = 50) -> MultiLineString:
    ...
    I, J = indices[:-1], [*indices[1:-1], len(original.geoms)+1]
    try:
        return MultiLineString([original.geoms[i:j+1].geoms for i, j in zip(I, J)])
    except:
        assert False, indices
...
def st_udf(sdf:SparkDataFrame, fn:callable, geometry_column:str="geometry"):
    ...
    """
    return (sdf
        .withColumn(geometry_column, F.expr(f"ST_AsBinary({geometry_column})"))
...
ax = plot_gdf(gdf, color="darkgoldenrod", alpha=.3, edgecolor="k")
