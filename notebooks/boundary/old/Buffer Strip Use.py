...
gdf = (
    sdf0.filter(sql_startswiths("id_parcel", ["NY9170", "NY9171", "NY9270", "NY9271"]))
    .select(
        "id_business",
        "id_parcel",
...
ax3.axis("off")
fig.tight_layout()
fig.legend(loc="center", framealpha=0)
...
n1, n2, n3 = (
    df["sum(m_not_hedge)"].sum(),
    df["sum(m_not_water)"].sum(),
    df["sum(m_not_either)"].sum(),
)
