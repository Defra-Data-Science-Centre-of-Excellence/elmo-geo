...
df_nca = df_nca.sort_values("proportion", ascending=False).drop_duplicates(subset=["id_parcel"]).drop(columns=["proportion"])
df_nca
...
colors = [[plt.get_cmap("Reds", lut=5)(x) for x in np.linspace(0, 1, 5)][x] for x in mapclassify.Quantiles(polygons.bare_soil_percent, k=5).yb]

n = 20
axes[0, 1].sharex(axes[1, 1])
head = polygons.head(n)
barcont = axes[0, 1].barh(y=head.nca_name, width=head.bare_soil_percent, color=colors[:n], ec="black", lw=0.5)
...
barcont = axes[1, 1].barh(y=tail.nca_name, width=tail.bare_soil_percent, color=colors[-n:], ec="black", lw=0.5)
...
fig.suptitle(
    (f"Mean bare soil % for land parcels in England by National " f"Character Area, November {year-1} - February {year}"),
    x=0,
    ha="left",
    fontsize="x-large",
)
