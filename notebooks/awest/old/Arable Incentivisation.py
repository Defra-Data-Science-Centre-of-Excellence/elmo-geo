...
sdf_parcel = sdf_wfm.selectExpr(
    ...
    "ha_arable + ha_grassland AS ha_total",
)

sdf_business = sdf_parcel.groupby("id_business").agg(F.expr("SUM(ha_arable) / SUM(ha_arable + ha_grassland) AS p_arable"))

sdf = sdf_parcel.join(sdf_business, on="id_business").selectExpr(
    "id_business",
    "id_parcel",
...
df = pd.concat(
    sdf.filter(f"{arable_ness} <= p_arable")
    .withColumn(
        "gmph_arable",
        F.expr("CASE WHEN 0==gmph_arable THEN null ELSE gmph_arable END" if null_fallow else "gmph_arable"),
    )
    .groupby("id_business")
    .agg(
    ...
    .agg(
        ...
        F.expr("MEAN(max_all) AS p_all"),
        F.expr("SUM(max_all) AS n_all"),
        F.expr("MEAN(min_none) AS p_none"),
        F.expr("SUM(min_none) AS n_none"),
    )
    .toPandas()
