...
import elmo_geo
from elmo_geo.plot import osm_provider
...
gdf_countries = gpd.read_file('/dbfs/FileStore/ons_countries_2022.gpkg').to_crs('epsg:3857').query('CTRY22NM == "England"').assign(geometry=lambda df: df.simplify(0.001))
...
for ax, col in zip(axs.flat, cols_bin):
    ax = df.plot(ax=ax, column=col, legend=True, cmap=cmap, edgecolor='k', linewidth=.5, alpha=1, figsize=(9,16))
...
axs = np.array([None]*3)
for ax, col in zip(axs.flat, cols_bin):
    ax = df.plot(ax=ax, column=col+' (std)', legend=True, cmap=cmap, edgecolor='k', linewidth=.5, alpha=1, figsize=(9,16))
    ax = gdf_countries.plot(ax=ax, facecolor='none', edgecolor='k', linewidth=1)
    ax.set(title=col)
    ax.axis('off')
    ax.get_legend().set(title='Farm Area')
...
axs = np.array([None]*3)
for ax, col in zip(axs.flat, cols_bin):
    ax = df.plot(ax=ax, column=col, legend=True, cmap=cmap, edgecolor='k', linewidth=.5, alpha=.7, figsize=(9,16))
    # ax = gdf_countries.plot(ax=ax, facecolor='none', edgecolor='k', linewidth=1)
    ax.set(title=col)
    ax.axis('off')
    ax.get_legend().set(title='Farm Area')
