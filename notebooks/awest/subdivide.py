...
# MAGIC %md
# MAGIC # Subdivide
# MAGIC Explore subdividing algorithms, useful large geometries.
# MAGIC Single geometries can be so large they cause OOM errors.
# MAGIC By subdividing them, the geometries can be analysed separately on partitions.
# MAGIC Examples are datasets like flood warning areas, where many organisations provide a single geometry to a large dataset,
# MAGIC some of these geometries are for areas around whole river basins.
# MAGIC Another example is none generalised country geometries.
...
# MAGIC %py
# MAGIC %timeit df['geometry'].transform(fishnet, threshold=100)
...
# MAGIC %py
# MAGIC %timeit df['geometry'].transform(katana, threshold=original_threshold, geometry_split=geometry_split_box)
# MAGIC %timeit df['geometry'].transform(katana, threshold=vertices_threshold, geometry_split=geometry_split_box)
# MAGIC %timeit df['geometry'].transform(katana, threshold=vertices_threshold, geometry_split=geometry_split_line)
# MAGIC %timeit df['geometry'].transform(katana, threshold=vertices_threshold, geometry_split=geometry_split_rect)
...
# MAGIC %py
# MAGIC %timeit df['geometry'].transform(delaunay)
