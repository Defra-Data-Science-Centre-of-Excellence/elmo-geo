...
def da_clip_mean(col, da):
  ...
  def _udf(geometry):
    ...
    except (OneDimensionalRaster, NoDataInBounds) as e:
      return
  return _udf(col)
...
sdf_fert = (sdf_parcels
  .withColumn('_out', da_clip_mean('geometry', da_fert))
  .select('id_parcel', *[
    F.expr(f'_out[{i}] AS {name}')
    for i, name in enumerate(da_fert.band.data)
  ])
)
...
sdf_pest = (sdf_parcels
  .withColumn('_out', da_clip_mean('geometry', da_pest))
  .select('id_parcel', *[
    F.expr(f'_out[{i}] AS {name}')
    for i, name in enumerate(da_pest.band.data)
  ])
)
...
df = (df_wfm
  .merge(df_fert, on='id_parcel', how='outer')
  .merge(df_pest, on='id_parcel', how='outer')
)
...
def rasterio_window_open():
  with rasterio.open(filepath) as src:
    _, _, window = rasterio.mask.raster_geometry_mask(src, [geometry], crop=True)
    return src.read(window=window, masked=True)

def rasterio_mask_open():
  with rasterio.open(filepath) as src:
    data, _ = rasterio.mask.mask(src, [geometry], crop=True, nodata=np.nan)
  return data
