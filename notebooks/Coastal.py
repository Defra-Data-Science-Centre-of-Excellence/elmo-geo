...
df = pd.DataFrame.merge(
  df_ph,
  df_wfm[['id_business', 'id_parcel', 'gm_parcel', 'ha_parcel_geo', 'ha_parcel_permanent_crops', 'ha_parcel_permanent_pasture']],
  on = 'id_parcel',
  how = 'outer',
)

threshold = .5 * df['ha_parcel_geo']
df['arable_grass'] = (
  np.where(threshold < df['ha_parcel_permanent_crops'], 'arable',
  np.where(threshold < df['ha_parcel_permanent_pasture'], 'grassland',
  'mixed',
)))
...
coastal = ['Coastal saltmarsh', 'Coastal sand dunes', 'Coastal vegetated shingle', 'Mudflats', 'Saline lagoons', 'Maritime cliff and slope']
