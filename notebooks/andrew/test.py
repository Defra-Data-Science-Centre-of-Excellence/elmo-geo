...
sdf = (spark.read.parquet(sf)
  .withColumn('geometry', F.expr('ST_AsBinary(ST_Transform(ST_GeomFromWKB(geometry), "EPSG:27700", "EPSG:32630"))'))
  .withColumn('key2', F.floor(F.row_number().over(W.Window.partitionBy('tile').orderBy('id_parcel'))/batchsize))
)
