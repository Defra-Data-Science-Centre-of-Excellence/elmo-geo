...
def batch_folder(path_in, ext, path_out, batch):
  def out(f_in):
    ...
    return f_out
  return pd.concat(
    batch_file(f, out(f), batch)
    for f in glob(path_in + '*' + ext)
  )
...
from cdap_geo.sedona import F, st_register, st_fromwkb, st_join
st_register()
...
sdf_geom = st_join(
  ...
  spark.read.parquet(sf_water).select(
    'theme', 'description', 'watertype', 'capturespecification',
    st_fromwkb('geometry'),
...
sdf = (spark.read.parquet(sf_geom)
  .groupby('id_business', 'id_parcel').agg(
