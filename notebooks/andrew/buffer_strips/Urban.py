...
extent = osdatahub.Extent.from_bbox([0, 0, .7e6, 1.3e6], crs)
...
from cdap_geo.sedona import st_register, st_join, st_load
st_register()
...
sdf_urban = spark.read.parquet(sf_urban).select(
...
aggr_geometry_urban_of = lambda urban_type: f'''
  ST_Union_Aggr(CASE
...
sdf_geom = (st_join(sdf_parcel, sdf_urban, lsuffix='_parcel', rsuffix='_urban')
  .groupBy('id_parcel')
  .agg(
