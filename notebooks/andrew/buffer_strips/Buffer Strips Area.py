...
from cdap_geo.sedona import st_register, st, st_load, st_join
st_register()
...
sdf_water = (spark.read.parquet(sf_os).select('geometry')
  .union(spark.read.parquet(sf_osm).select('geometry'))
  .select(
    st_load('geometry').alias('geometry'),
  )
  .withColumn('geometry', F.expr(f'ST_MakeValid(ST_Buffer(geometry, {WATERBODY_BUFFER_CROSS_COMPLIANCE}))'))
  .transform(lambda right: st_join(
  ...
  .withColumn('geometry', F.expr('ST_MakeValid(ST_Intersection(ST_MakeValid(ST_Boundary(geometry)), geometry_left))'))
  .drop('geometry_left')
  .repartition(200, 'id_parcel')
)
...
df['geometry_parcel'].plot(ax=ax, color='darkgoldenrod', alpha=.6, edgecolor='darkgoldenrod').set(title='BNG: NY9170-NY9271')
...
for i, ax in enumerate(axs):
  df_parcel[i:i+1].plot(ax=ax, color='C1', alpha=.5)
  df_buf4[i:i+1].plot(ax=ax, color='C4', alpha=.25)
  df_buf12[i:i+1].plot(ax=ax, color='C0', alpha=.25)
