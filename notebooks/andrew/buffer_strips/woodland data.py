...
df_living_england_woodland_points = ( df_living_england
  .filter(A | B)
  .withColumn('A_prob', F.when(F.col('A_prob')==0, 100).otherwise(F.col('A_prob')))
  .withColumn('prob', F.when(A, F.col('A_prob')).when(B, F.col('B_prob')).when(A&B, F.col('A_prob')+F.col('B_prob')).otherwise(0) / 100)
...
# Living England
df_living_england_woodland_points = spark.read.parquet('dbfs:/mnt/lab/unrestricted/geoparquet/natural_england/living_england_woodland_points/2022_10_10.parquet/')
le = df_living_england_woodland_points \
  .select('area', 'geometry')


# Ancient Woodland
aw = df_ancient_woodland \
  .select('geometry')


# Traditional Orchards
to = df_traditional_orchards \
  .select('geometry')


# Community Forests
cf = df_community_forests \
  .select('geometry')


# Felling Licences
fl = df_felling_licences \
  .filter(F.col('Descriptr').isin([
...
nfi = df_national_forest_inventory.filter(
  (F.col('COUNTRY') == F.lit('England')) &
  (F.col('CATEGORY') == F.lit('Woodland'))
).select(
...
df2 = df.withColumn('geometry', F.expr('ST_Difference(ST_GeomFromWKB(hex(geometry_left)), ST_Union(ST_GeomFromWKB(hex(geometry_right))))'))
display(df2)
...
for right, name, sign in zip(
  [le, aw, to, cf, fl, nfi],
  ['Living England', 'Ancient Woodland', 'Traditional Orchards', 'Community Forests', 'Felling Licence', 'National Forest Inventory'],
  [+1, +1, +1, +1, -1, -1],
):
  if name == 'Living England':
    right = right.withColumn('geometry', cdap_geo.buffer('geometry', r))
  df = st_join(parcels, right, from_wkb=True, lsuffix='_parcel', rsuffix='') \
...
df2 = df.groupBy('id_business', 'id_parcel').agg(
