...
from cdap_geo.sedona import st_register, st_load, st_join, st_valid
st_register()
...
sdf_geoms = (st_join(
    df_parcel,
    (df_wall
      .withColumnRenamed('geometry', 'geometry_wall')
      .withColumn('geometry', F.expr('ST_Buffer(geometry_wall, 2)'))
    ),
    lsuffix = '_parcel',
    rsuffix = '',
  )
...
sdf = (spark.read.parquet(sf_geoms)
