...
gdf_sssi = (gpd.read_file(f_sssi)
  .to_crs(27700)
  [['OBJECTID', 'SSSI_NAME', 'CONDITION', 'geometry']]
  .rename(columns={'OBJECTID': 'OBJECTID_sssi'})
)
gdf_commons = (gpd.read_file(f_commons)
  .to_crs(27700)
  [['OBJECTID', 'geometry']]
...
df = (
  ...
  .assign(
    ...
    condition_code = lambda df: pd.Categorical(df['condition'], categories=[
      'Favourable',               # 0
