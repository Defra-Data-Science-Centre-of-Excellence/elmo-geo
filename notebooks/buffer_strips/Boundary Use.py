...
buf = 12
sdf_adj = (elm_se.st.sjoin(
    sdf_parcel,
    sdf_parcel, distance=12,
...
sdf_adj = (spark.read.parquet(sf_adj)
  .groupby('id_parcel')
  .agg(st_union('geometry_adj'))
)

sdf_water = (spark.read.parquet(sf_os_water)
...
sdf_ditch = (spark.read.parquet(sf_os_water)
  .filter('description == "Drain"')
...
sdf_wall = (spark.read.parquet(sf_wall)
  .groupby('id_parcel')
...
sdf_neighbour = (sdf_parcel
  ...
  .select(
    'id_parcel',
    *[elm_se.io.load_missing(col).alias(col) for col in [
      'geometry_boundary', 'geometry_adj',
      'geometry_water', 'geometry_ditch',
      'geometry_wall',
      'geometry_hedge',
    ]],
  )
...
boundary_use = lambda sdf, use, buf: (sdf
  .withColumn('tmp', F.expr(f'ST_Buffer(geometry_{use}, {buf})'))
  .withColumn('tmp', F.expr('''EXPLODE(Array(
...
sdf_boundary = (spark.read.parquet(sf_neighbour)
...
sdf_type = (pd.read_feather(f_wfm_farm)
...
sdf_ha = (spark.read.parquet(sf_parcel)
...
sdf_uptake = (sdf_type
  .join(sdf_ha, on='id_business', how='full')
  .join(sdf_m, on='id_parcel', how='full')
  .join(sdf_ph, on='id_parcel', how='full')
  .join(sdf_evast, on='id_parcel', how='full').withColumn('woodland', F.expr('COALESCE(woodland, False)'))
  .join(sdf_peat, on='id_parcel', how='full').withColumn('peatland', F.expr('COALESCE(peatland, 0)'))
  .join(sdf_wet, on='id_parcel', how='full').withColumn('wetland', F.expr('COALESCE(wetland, 0)'))
  .select(
    'id_business', 'id_parcel', 'farm_type', 'priority_habitat',
    'elg_adj', 'elg_water', 'elg_ditch', 'elg_wall', 'elg_hedge',
