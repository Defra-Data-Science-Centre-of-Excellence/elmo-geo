...
# Drop ST_Points
sdf_os_water = (sdf_os_water
  .filter('ST_GeometryType(geometry) != "ST_Point"')
  .withColumn('geometry', F.expr('EXPLODE(ST_Dump(geometry))'))
  .transform(re_part)
)

print('OS Water (no st_points):',
  sdf_os_water.rdd.getNumPartitions(),
  sdf_os_water.count(),
  sdf_os_water.filter('geometry IS NULL').count(),
  sdf_os_water.filter('NOT ST_IsValid(geometry)').count(),
...
sdf_geom = join(
  ...
  'osid', 'dataset', 'description',
  'geometry_parcel', 'geometry_water',
).transform(re_part)


sdf_geom.write.parquet(sf_geom, mode='overwrite')
print('Spatially Joined:',
...
bufs = 4, 6, 8, 10, 12
sdf_geom = (spark.read.parquet(sf_geom)
  .withColumn('geometry_water', F.expr('ST_MakeValid(ST_Buffer(geometry_water, 0.001))'))  # LineStrings into Polygons for ST_Union_Aggr
)

ha_water = lambda buf: F.expr(f'''
...
m_boundary = lambda buf: F.expr(f'''
