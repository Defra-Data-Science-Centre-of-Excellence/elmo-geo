...
df = (spark.read.parquet(sf_neighbour)
...
gdf = (df
  .assign(**{
    col: gpd.GeoSeries.from_wkb(df[col]).set_crs(27700)
    for col in cols
  })
  .pipe(gpd.GeoDataFrame, geometry='geometry_boundary', crs=27700)
)
...
gdf = (gpd.read_parquet('/dbfs/tmp/awest/boundaries.parquet')
  ...
  .set_geometry('geometry').set_crs(27700)
)
...
plt.gca().set(title='Read/Write speed of Formats', xlabel='data: ~1000 features of many geometry types', ylabel='Time (s)')
...
gdf_neighbour['geometry_adj'].buffer(8).plot(ax=ax, color='goldenrod', alpha=.4, linewidth=4)
...
gdf_neighbour['geometry_water'].plot(ax=ax, color='tab:blue', alpha=.4, linewidth=4)
...
gdf_neighbour['geometry_hedge'].plot(ax=ax, color='tab:green', alpha=.4, linewidth=4)
gpd.GeoSeries(shapely.LineString([[446521, 349767], [446550, 349740]])).plot(ax=ax, color='tab:green', alpha=.4, linewidth=4)
...
sdf = (sdf_uptake
  .filter('elg_ditch AND 0.1<peatland')
